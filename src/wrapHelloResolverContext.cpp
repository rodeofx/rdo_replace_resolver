...
static std::string
_Repr(const HelloResolverContext& ctx)
{
...
static size_t
_Hash(const HelloResolverContext& ctx)
{
    return hash_value(ctx);
}

void
wrapHelloResolverContext()
{
    using This = HelloResolverContext;

    class_<This>
        ("HelloResolverContext", no_init)
        .def(init<>())
